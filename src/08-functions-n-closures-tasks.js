...
function getPolynom(...args) {
  const reversed = [0, 0, ...args].reverse();
  return (x) => (args.length === 0
    ? null
    : reversed[0] + (reversed[1] * x) + (reversed[2] * (x ** 2)));
}
...
function logger(func, logFunc) {
  return (...args) => {
    const argsPrint = args.map((x) => JSON.stringify(x)).join();
    logFunc(`${func.name}(${argsPrint}) starts`);
    try {
      return func(...args);
    } finally {
      logFunc(`${func.name}(${argsPrint}) ends`);
    }
  };
}
